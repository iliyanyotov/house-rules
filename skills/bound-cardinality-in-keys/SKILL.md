...
**A cache is not a metric.** For a metric label or error tag, every distinct value is a permanent series that bills forever, so the bar is "bounded value space." A cache entry *self-evicts*, so the bar is different: **will this key be re-read before it falls out?** That's why `invoice:${invoiceId}` and `user-timezone:${userId}` are *correct* — one entry per entity, re-read on every request for that entity. The leak is keying on an unbounded *combination of request parameters* (timestamps + free text + email) where each key is written once and never read again. An unbounded entity ID in a cache key is fine; an unbounded parameter-tuple is the leak.

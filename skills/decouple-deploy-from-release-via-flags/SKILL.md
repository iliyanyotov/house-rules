...
- **Infrastructural**: this binary will run, this schema will be in place, these env vars are set. *Atomic on Vercel.*
...
This is core to the Accelerate / DORA framing of "deployment frequency" being a *leading* indicator of organizational health: teams that decouple deploy from release deploy more often *and* break things less often, because the deploys are smaller, mostly-benign, and recoverable independent of CI.

It's also what makes [[expand-contract-schema-migration]] tractable — the "expand" deploy is benign because the new code path is flagged off. The "contract" deploy happens after the flag is fully on.
...
- The team coordinates deploy-and-go-live via Slack message instead of a flag-flip control plane.
...
[cleanup]      — old code deleted (see dead-code-deletion-on-sight); flag removed; type narrows to just 'v2-full'.
```
...
async function placeOrder(input: CheckoutInput): Promise<Order> {
  switch (checkoutMode) {
    ...
    case 'v2-shadow': {
      ...
      placeOrderV2Shadow(input).then(shadow =>
        logShadowDiff(real, shadow)
...
The exhaustive-switch (see [[exhaustive-switch]]) guarantees every flag state is handled. Removing a state at cleanup time becomes a compile-error-driven refactor — every consumer is forced to acknowledge the change.

### Where the flag value comes from

Three options in order of capability:

1. **Env var** (`CHECKOUT_MODE`) — simplest. Flipping requires a Vercel env change + re-deploy of *one* env value, which is faster than a full code re-deploy but not instant.

2. **Vercel Edge Config** — flags live in a low-latency KV; reading is a sub-millisecond fetch. Flipping is *instant* — no deploy. Good for runtime-toggleable flags.

3. **Dedicated flag platform** (LaunchDarkly, Statsig, Flagsmith, Vercel's own Flags SDK) — adds per-user targeting, gradual rollout, contextual rules. Worth it once you have multiple flags in flight.
 ...
 *   - v2-canary   → 1% of traffic uses v2 (set in Edge Config rule)
...
Every flag has a documented end state. After the feature is fully released and stable for ~one cycle (two weeks, a month — set explicitly), the flag and the old code path are *deleted*. See [[dead-code-deletion-on-sight]].
...
Plus: delete `placeOrderV1`, delete the `CheckoutMode` type, delete the env var from Vercel. Ship as its own small PR (see [[small-changesets]]).
...
The smallest flag system is a typed env var. That's one file (the parsing wrapper) and zero infrastructure. It still gives you the deploy/release split. The over-engineered version (LaunchDarkly + targeting + per-user variants) is for *later*. The *discipline* of decoupling is for *now*.
...
### "Vercel rollback is fast enough"

A Vercel rollback rolls back *the whole deploy* — including unrelated fixes shipped alongside. That's the coupling this rule is breaking. The flag flip rolls back *only the misbehaving feature*.

### "Flags add complexity to the code"

A `switch` on a typed union is the same shape as any discriminated state we already use. The complexity is bounded, named, and removed when the flag is cleaned up. The alternative complexity (re-deploys, multi-step coordinated launches, "wait for ops to flip the lever") is *worse* and *unbounded*.

### "We don't have flag infrastructure"

You have env vars. Start there. Migrate to Edge Config when you need runtime flip; migrate to a flag platform when you need per-user targeting. The discipline scales independently of the tooling.
...
- A PR description that includes "deploy plan: announce in Slack, then merge and watch metrics."
...
- A flag whose default is on but the code still has the off branch — old branch is dead code; see [[dead-code-deletion-on-sight]].
...
| "Vercel rollback is fast" | Rollback is whole-deploy. Flags are per-feature. Different granularities. |
...
| "We don't have LaunchDarkly" | Env vars work. Edge Config works. The tool is replaceable; the discipline isn't. |
| "Reverting is good enough" | Reverting reverts *everything*. Flags revert *one thing*. Different scopes. |

## Reference

Forsgren, Humble, Kim, *Accelerate* (2018) — the DORA finding that *deployment frequency* and *change failure rate* both improve when deploys and releases are decoupled. The high-performing teams in the State of DevOps reports all do this.
...
Martin Fowler, [*Feature Toggles*](https://martinfowler.com/articles/feature-toggles.html) — the canonical write-up. Distinguishes release toggles, experiment toggles, ops toggles, and permission toggles. The release-toggle category is what this skill is about.

Pete Hodgson, *Feature toggles are one of the worst kinds of technical debt* — the cautionary version. Flags are a *tool*; without removal discipline, they're debt. This skill's removal-plan half is what addresses it.

Vercel Edge Config, Vercel Flags SDK — the platform's first-class support for runtime-toggleable flags.

Adjacent rules: [[expand-contract-schema-migration]] (the schema-side analog; the flag is what gates the code-path side of the same multi-deploy sequence), [[exhaustive-switch]] (the typed flag's consumer pattern — adding/removing states becomes a compile-error-driven refactor), [[dead-code-deletion-on-sight]] (when the flag is fully rolled out, the old code path is dead — delete it, don't leave it "just in case"), [[small-changesets]] (each rollout stage is its own small PR), [[graceful-degradation-defaults]] (a flag flipped to a degraded mode under incident is shedding behavior, not features — combines with this rule), [[fail-fast]] (the flag default is *safe* — when the env var is missing or invalid, the system falls back to the known-good behavior, not the new one).

...
Production systems make this a first-class field — a `type` enum on the flag row (`RELEASE | EXPERIMENT | OPERATIONAL | KILL_SWITCH | PERMISSION`). The staleness rule and its red flag below apply to **release and experiment** toggles only — the rest are not bugs for living long. Every other red flag (typing, scoping, ownership, rollback) applies to all flag types.
